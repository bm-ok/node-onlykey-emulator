...
Runs the [OnlyKey firmware](../OnlyKey-Firmware) as a Node.js native addon,
...
* The firmware sources and Teensyduino toolchain, checked out **beside** this
  repo (see [Where the components live](#where-the-components-live) and `setup.sh`)

### Where the components live

This repo does not contain the components — it sits **beside** them. Nine repos
share one folder:

```
<workspace>/
  node-onlykey-emulator/          this repo
  ...
  OnlyKey-App/                    the NW.js app
  okpqc-venv/                     generated by setup.sh, beside the repos
```

Each component is a **swap slot**, not a vendored dependency: any of them can be
substituted wholesale — a different fork, an upstream revision, a branch under
test — without this repo changing at all. What is resolved is the *directory
name*, so a checkout under one of those names is used whatever its origin.

Because they are separate checkouts you work on directly, `setup.sh` never
fetches, pulls or re-clones into one that is already there. It only clones what
is missing, and only when asked — see [One-time setup](#1-fetch-sources-and-dependencies).
Nothing is pinned; if you want a reproducible set, tag the component repos rather
than pinning them here — a recorded known-good combination stays advisory, where
a pin in this repo would defeat the swapping.

`okpqc-venv/` is *generated* from those checkouts, so it is an output rather than
a source, and it belongs beside them rather than inside any one of them: it is
not a repo, and it must not sit in one anybody commits from. That is also where
the test kit looks for it — onlykey-testing resolves `okpqc-venv/bin` against the
parent of its own checkout.
...
match it**. Since none of the component checkouts may be changed to accommodate
the emulator, the emulator has to present those fields for real.
...
Provisions `okpqc-venv` from the eight component checkouts beside this repo
(including `age`/`age-keygen`, which pip cannot supply), builds the firmware
toolchain image if Docker is present, and builds the emulator addon.

Components already there are used **as they are** — never fetched, pulled or
re-cloned. If any are missing it names them and stops. To have it clone the
missing ones into that same folder, beside this repo:

```sh
./setup.sh --clone
```

That is also the path from a bare start: clone only this repo, then
`./setup.sh --clone` fills in the other eight around it. Every clone tracks its
default branch — nothing is pinned, so a component stays swappable.

Re-running is safe: existing checkouts and an existing venv are left alone.
...
of watching its presses vanish. `onlykey-testing` was updated to match
...
```
grep -rn OK_EMULATOR ../OnlyKey-Firmware ../libraries
```

**Nothing in the component checkouts is written to by the build.**
